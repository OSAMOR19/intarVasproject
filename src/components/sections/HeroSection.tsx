...
const HeroSection = ({
  ...
  return (
    <section ref={sectionRef} className="auto pt-18 lg:pt-16 lg:h-[95vh] overflow-hidden" style={{ backgroundImage: 'url(/images/herosectionbg.svg)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <div className="container h-full mx-auto grid gap-10 px-4 lg:px-0 pt-20 lg:pt-0 lg:py-4 lg:grid-cols-2 lg:items-center relative z-10">
        ...
        <div className="flex flex-col items-start pt-20 lg:pt-0 justify-center space-y-8 lg:pl-4">
          <h1 className={`max-w-lg text-[32px] sm:text-[40px] md:text-[48px] lg:text-[54px] font-inter text-white font-[800] tracking-tight transition-all duration-1000 ${
            showTitle ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-8'
          }`}>
            {title}
          </h1>
          <p className={`mt-4 text-[18px] text-[#C2C6CE] max-w-lg text-muted-foreground transition-all duration-1000 ${
            showSubtitle ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-8'
          }`}>
            {subtitle}
          </p>
           <div className={`mt-8 flex flex-col sm:flex-row gap-3 w-full sm:w-auto sm:max-w-none transition-all duration-1000 ${
